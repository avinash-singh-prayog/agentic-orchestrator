"""
Booking Agent A2A Server Wrapper.
...
class BookingAgentExecutor(AgentExecutor):
    """
    Message-based executor for the booking agent.
    ...
    def __init__(self):
        ...
        self.agent_card = BookingAgentCard.model_dump(mode="json", exclude_none=True)
    ...
    async def execute(self, context: RequestContext, event_queue: EventQueue) -> None:
        ...
        logger.info(f"Executing booking agent with prompt: {prompt[:100]}...")
        ...
        try:
            ...
            message = Message(
                messageId=str(uuid4()),
                role=Role.agent,
                metadata={"name": self.agent_card.get("name", "Booking Agent")},
...
async def run_server():
    """
    Run the Booking Agent using SLIM transport pattern.
    
    This follows the guide's AppContainer + AppSession pattern for SLIM communication.
    """
    logger.info("Initializing Booking Agent server...")
    ...
    server = A2AStarletteApplication(
        agent_card=BookingAgentCard,
        http_handler=request_handler
    )
    
    # 4. Create SLIM transport
    slim_endpoint = os.getenv("SLIM_ENDPOINT", "http://orchestrator-slim:46357")
    # Use explicit topic matching the AgentCard id
    personal_topic = "booking-agent"  # Matches the id from AgentCard
    ...
    logger.info(f"Starting booking agent on SLIM with topic: {personal_topic}")

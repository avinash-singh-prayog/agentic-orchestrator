"""
Serviceability Agent A2A Server Wrapper.
...
# Internal Imports
from ..domain.card import ServiceabilityAgentCard
...
class ServiceabilityAgentExecutor(AgentExecutor):
    ...
    def __init__(self):
        ...
        self.agent_card = ServiceabilityAgentCard.model_dump(mode="json", exclude_none=True)
    ...
    async def execute(self, context: RequestContext, event_queue: EventQueue) -> None:
        ...
        logger.info(f"Executing serviceability agent with prompt: {prompt[:100]}...")
        ...
        try:
            ...
            message = Message(
                messageId=str(uuid4()),
                role=Role.agent,
                metadata={"name": self.agent_card.get("name", "Serviceability Agent")},
...
async def run_server():
    """
    Run the Serviceability Agent using SLIM transport pattern.
    
    This follows the guide's AppContainer + AppSession pattern for SLIM communication.
    """
    logger.info("Initializing Serviceability Agent server...")
    
    # 1. Create task store and executor
    task_store = InMemoryTaskStore()
    executor = ServiceabilityAgentExecutor()
    ...
    server = A2AStarletteApplication(
        agent_card=ServiceabilityAgentCard,
    ...
    logger.info(f"Starting serviceability agent on SLIM with topic: {personal_topic}")

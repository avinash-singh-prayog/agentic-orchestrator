"""
Transaction RCA Agent A2A Server Wrapper.
...
# Internal Imports
from domain.card import TransactionRCAAgentCard
from app.container import Container

logger = logging.getLogger("transaction_rca_agent.server")

# Initialize factory with tracing
factory = AgntcyFactory("orchestrator.transaction_rca_agent", enable_tracing=False)


class TransactionRCAAgentExecutor(AgentExecutor):
    """
    ...
    def __init__(self):
        ...
        self.agent_card = TransactionRCAAgentCard.model_dump(mode="json", exclude_none=True)
    ...
    async def execute(self, context: RequestContext, event_queue: EventQueue) -> None:
        ...
        try:
            ...
            message = Message(
                messageId=str(uuid4()),
                role=Role.agent,
                metadata={"name": self.agent_card.get("name", "Transaction RCA Agent")},
...
async def run_server():
    """
    Run the Transaction RCA Agent using SLIM transport pattern.
    
    This follows the guide's AppContainer + AppSession pattern for SLIM communication.
    """
    logger.info("Initializing Transaction RCA Agent server...")
    
    # 1. Create task store and executor
    task_store = InMemoryTaskStore()
    executor = TransactionRCAAgentExecutor()
    ...
    server = A2AStarletteApplication(
        agent_card=TransactionRCAAgentCard,
        http_handler=request_handler
    )
    
    # 4. Create SLIM transport
    slim_endpoint = os.getenv("SLIM_ENDPOINT", "http://orchestrator-slim:46357")
    # Use explicit topic matching supervisor client's TRANSACTION_RCA_AGENT_TOPIC
    personal_topic = "transaction-rca-agent"  # Matches the id from AgentCard
    ...
    logger.info(f"Starting transaction RCA agent on SLIM with topic: {personal_topic}")

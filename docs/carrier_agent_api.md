# Carrier Agent API Documentation

The Carrier Agent is a worker agent that handles carrier operations: serviceability checks, rate quotes, and shipment booking. It can be accessed directly via HTTP or through SLIM Transporter.
...
| Container Network | `http://carrier-agent:8000` |
...
Execute the carrier agent with a natural language prompt. The agent uses LangGraph to parse the request, check serviceability, and generate a response.

**Endpoint:** `POST /carrier/v1/agent/run`
...
**Example - Check Rates:**
```bash
curl -X POST http://localhost:9003/carrier/v1/agent/run \
...
**Endpoint:** `POST /carrier/v1/serviceability`
...
```bash
curl -X POST http://localhost:9003/carrier/v1/serviceability \
  -H "Content-Type: application/json" \
  -d '{
    "source_location": {
      "postal_code": "193501",
...
Check if the carrier agent is running and list available carriers.

**Endpoint:** `GET /carrier/v1/health`
...
**Example:**
```bash
curl http://localhost:9003/carrier/v1/health
```
...
The Carrier Agent supports two running modes:
...
```bash
# Local
uv run --package carrier-agent uvicorn carrier_agent.app.main:app --host 0.0.0.0 --port 8000

# Docker
docker compose up carrier-agent  # Without SLIM mode
```
...
```bash
# Docker (default)
docker compose up carrier-agent  # Uses "slim" argument

# Direct
python -m carrier_agent.app.main slim
```
...
|----------|-------------|---------|
| `CARRIER_AGENT_LLM` | LLM model to use | `openrouter/openai/gpt-4o-mini` |
